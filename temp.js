...
const DEFAULT_NAVBAR_ELEMENTS = [
  ...
  { id: 'reception', label: 'R�ception', icon: 'fa-cubes', visible: true, order: 2 },
  ...
  { id: 'livres', label: 'Livr�s', icon: 'fa-check-square', visible: true, order: 9 },
...
const DEFAULT_DROPDOWN_ITEMS = {
  ...
  'reception': [
    ...
    { id: 'colis-transferer', label: 'Colis � transf�rer', icon: 'fa-random', visible: true },
  ...
  'modification': [
    ...
    { id: 'demandes-traitees', label: 'Demandes trait�es', icon: 'fa-calendar', visible: true }
  ],
  'fdr': [
    { id: 'creer-fdr', label: 'Cr�er une feuille de route', icon: 'fe-file-text', visible: true },
  ...
  'livres': [
    { id: 'non-encaisse', label: 'Non Encaiss�s', icon: 'fa-clock', visible: true },
    { id: 'encaisse-non-verse', label: 'Encaiss�s non vers�s', icon: 'fa-dollar-sign', visible: true },
    { id: 'encaisse-verse', label: 'Encaiss�s et vers�s � la caisse', icon: 'fa-level-up-alt', visible: true },
    { id: 'ancien-historique', label: 'Ancien Historique', icon: 'fa-history', visible: true }
  ],
  'finance': [
    { id: 'reception-versement-admin', label: 'R�ception versement admin', icon: 'fa-check', visible: true, section: 'reception' },
    { id: 'depot-partenaire', label: 'D�pot d\'argent partenaire', icon: 'fa-user-plus', visible: true, section: 'reception' },
    { id: 'depenses-charges', label: 'D�penses (charges)', icon: 'fa-exchange-alt', visible: true, section: 'sortie' },
    ...
    { id: 'pvs-archives', label: 'PVs archiv�s', icon: 'fa-history', visible: true, section: 'payments' },
...
function getNavbarElementByIdentifier(element) {
  ...
  if ((text.includes('r�ception') || text.includes('reception')) && hasIcon('fa-cubes')) return 'reception';
  ...
  if (text.includes('livr�s') || text.includes('livres') || hasIcon('fa-check-square')) return 'livres';
...
function getDropdownElementByIdentifier(element, parentIdentifier) {
  ...
  switch(parentIdentifier) {
    ...
    case 'reception':
      ...
      if (text.includes('colis � transf�rer') || (text.includes('transf�rer') && hasIcon('fa-random'))) return 'colis-transferer';
    ...
    case 'modification':
      ...
      if (text.includes('demandes trait�es') || hasIcon('fa-calendar')) return 'demandes-traitees';
      break;
      
    case 'fdr':
      if (text.includes('cr�er une feuille') || hasIcon('fe-file-text')) return 'creer-fdr';
    ...
    case 'retours':
      ...
      if (text.includes('a dispatcher') && hasIcon('fa-random')) {
        ...
        if (sectionHeader && sectionHeader.textContent.includes('entrep�t')) return 'retours-dispatch-entrepot';
      ...
      if (text.includes('en transit') && hasIcon('fa-exchange-alt')) {
        ...
        if (sectionHeader && sectionHeader.textContent.includes('entrep�t')) return 'retours-transit-entrepot';
      ...
      if (text.includes('historique') && hasIcon('fa-history')) {
        const sectionHeader = element.previousElementSibling;
        if (sectionHeader && sectionHeader.textContent.includes('entrep�t')) return 'retours-historique-entrepot';
        if (sectionHeader && sectionHeader.textContent.includes('Partenaires')) return 'retours-historique-partenaires';
      }
      break;
      
    case 'livres':
      if (text.includes('non encaiss�s') || hasIcon('fa-clock')) return 'non-encaisse';
      if (text.includes('encaiss�s non vers�s') || (text.includes('non vers�s') && hasIcon('fa-dollar-sign'))) return 'encaisse-non-verse';
      if (text.includes('encaiss�s et vers�s') || hasIcon('fa-level-up-alt')) return 'encaisse-verse';
      if (text.includes('ancien historique') || (text.includes('historique') && hasIcon('fa-history'))) return 'ancien-historique';
      break;
      
    case 'finance':
      if (text.includes('r�ception versement admin') || (text.includes('versement') && hasIcon('fa-check'))) return 'reception-versement-admin';
      if (text.includes('d�pot d\'argent partenaire') || hasIcon('fa-user-plus')) return 'depot-partenaire';
      if (text.includes('d�penses (charges)') || (text.includes('d�penses') && hasIcon('fa-exchange-alt'))) return 'depenses-charges';
      ...
      if (text.includes('pvs archiv�s') || (text.includes('archiv�s') && hasIcon('fa-history'))) return 'pvs-archives';
